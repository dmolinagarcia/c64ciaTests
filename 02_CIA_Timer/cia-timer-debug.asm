...
restart:
  lda $d011
  bpl *-3       ; jump if raster bit #8 = 0 Wait for raster > 256
                ; This is in the border
  lda #$00
  sta $fa
nexttest:
  ; "clear" CIAs
  ; CRGs set to all 0. ICR read ack
  lda #$00
  sta $dc0e
  sta $dc0f
  sta $dd0e
  ...
  lda $dc0d
  lda $dd0d
  ; Restore VIC raster interrupt to default
  lda #$1b
  sta $d011
  jsr checkdata
  jmp *
...
clrlp:
  ...
  rts
;-------------------------------------------------------
; What it says, accurate 80-A cycle delay
; org = $1000 so we ensure we do not cross
; any page boundaries
;-------------------------------------------------------
  .org $1000
...
;-------------------------------------------------------
; Variables
; out: posiciones de salida en pantalla
; greet_msg: mensaje de bienvenida
;-------------------------------------------------------
out:
  .word $0450,$0518,$0464,$052c,$0608,$06d0,$061c,$06e4
greet_msg:
    dc.b 147,"  CIA-TIMERS DMG / REFERENCE: "
    if DUMP = 0
    dc.b "OLD"
    endif
    if DUMP = 1
